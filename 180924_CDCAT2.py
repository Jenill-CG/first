...
def create_attendance_pdf(pdf, column_widths, column_names, info_values, df, format_option):
    ...
    pdf.set_font('Arial', 'BIU', 10)
    pdf.cell(info_cell_width*0.08, 6," English ", border=0, ln=0,align="L")
    pdf.set_font('Arial', 'IU', 10)
    pdf.cell(info_cell_width*0.16, 6," BLOCK LETTERS", border=0, ln=0,align="L")
    pdf.set_font('Arial', 'I', 10)
    pdf.cell(info_cell_width*0.25, 6,"   only", border=0, ln=1, align="L")
    ...
    if format_option == 'Pen Paper Assessment':
        ...
        pdf.cell(83, 3, '', border='LTR', align='C', fill=True)
        ...
        pdf.ln(3)
        # First row of headers
        pdf.cell(34, 2.5, 'STUDENT ID',  border='LR', align='C', fill=True)
        pdf.cell(83, 2.5, 'STUDENT FULL NAME',  border='LR', align='C', fill=True)
        pdf.cell(16, 2.5, 'GENDER', border='LR', align='C', fill=True)
        pdf.cell(16, 2.5, 'SET CODE', border='LR', align='C', fill=True)
        pdf.cell(30, 2.5, 'PRESENT/ABSENT', border='LR', align='C', fill=True)
        ...
        pdf.cell(83, 4.5, '', border='LBR', align='C', fill=True)  # Empty cell under STUDENT NAME
...
def main():
    ...
    st.markdown("<div class='custom-header' style='background-color: #008000; text-align: center;padding: 15px; font-size: 40px;font-weight: bold;color: #FFFFFF; border-radius: 10px; box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.2);'>CDCAT G-3/4/5 ID Builder</div>", unsafe_allow_html=True)
    st.markdown("<div class='custom-header' style='font-size: 26px;color: #008000;text-align: right;padding: 2px; background-color: #ffffff; margin-top: 2px;margin-bottom: 2px;box-shadow: 0px 4px 8px rgba(0, 0, 0, 0);text-shadow: 1px 1px 2px rgba(0, 0, 0, 0);'>Generate unique IDs quickly and easily!</div>", unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        ...
        if run_default:
            ...
            states = ["ANDHRA PRADESH","MAHARASHTRA_1","TELANGANA","MAHARASHTRA_2","JHARKHAND", "HIMACHAL PRADESH", "UTTAR PRADESH"]
