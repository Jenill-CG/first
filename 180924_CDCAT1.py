...
def create_attendance_pdf(pdf, column_widths, column_names, info_values, df, format_option):
    ...
    if format_option == 'Pen Paper Assessment':
        ...
        pdf.cell(75, 2.5, 'STUDENT FULL NAME',  border='LR', align='C', fill=True)
        pdf.cell(16, 2.5, 'GENDER', border='LR', align='C', fill=True)
        pdf.cell(16, 2.5, 'SET CODE', border='LR', align='C', fill=True)
        pdf.cell(19, 2.5, 'PART A', border='LR', align='C', fill=True)
        pdf.cell(19, 2.5, 'PART B', border='LR', align='C', fill=True)
        # pdf.cell(12, 0.5, 'SECTION', border='LR', align='C')
        # pdf.cell(12, 0.5, 'SESSION', border='LR', align='C')  # End of the row
...
def main():
    ...
    st.markdown("<div class='custom-header' style='background-color: #0D98BA; text-align: center;padding: 15px; font-size: 40px;font-weight: bold;color: #FFFFFF; border-radius: 10px; box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.2);'>CDCAT G-6/7/8/9 ID Builder</div>", unsafe_allow_html=True)
    st.markdown("<div class='custom-header' style='font-size: 26px;color: #0D98BA;text-align: right;padding: 2px; background-color: #ffffff; margin-top: 2px;margin-bottom: 2px;box-shadow: 0px 4px 8px rgba(0, 0, 0, 0);text-shadow: 1px 1px 2px rgba(0, 0, 0, 0);'>Generate unique IDs quickly and easily!</div>", unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        ...
        if run_default:
            ...
            with col3:        
                grade = st.number_input("Grade", min_value=1, value=6)
