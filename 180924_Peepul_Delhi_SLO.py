...
parameter_descriptions = {
    ...
    'A3': "zone + School + Grade + Student",
    'A4': "Partner + School + Grade + Student",
    'A5': "zone + Block + School + Grade + Student",
    'A6': "Partner + Block + School + Grade + Student",
    'A7': "Partner + zone + School + Grade + Student",
    'A8': "Partner + zone + Block + School + Grade + Student"
}
...
def create_attendance_pdf(pdf, column_widths, column_names, image_path, info_values, df, format_option):
    ...
    pdf.cell(info_cell_width, 3.75, f"ZONE : {info_labels['ZONE']}", border='LR', ln=1)
    # pdf.cell(info_cell_width, 3, f"BLOCK : {info_labels['BLOCK']}", border='LR', ln=1)

    # Add the SCHOOL NAME
    pdf.cell(school_name_width, 3.75, f"SCHOOL NAME : {info_labels['SCHOOL NAME']}", border='L', ln=0)  # Left border only
    ...
    pdf.cell(info_cell_width*0.6, 3.75, f"CLASS : {info_labels['CLASS']}", border='L', ln=0)
    pdf.cell(info_cell_width*0.4, 3.75, "TOTAL PRESENT STUDENTS : _____", border='R', ln=1, align='R')
    
    pdf.cell(info_cell_width*0.6, 3.75, f"UDISE CODE : {info_labels['UDISE CODE']}", border='L', ln=0)
    pdf.cell(info_cell_width*0.4, 3.75, " TOTAL ABSENT STUDENTS : _____", border='R', ln=1, align='R')
    ...
    if format_option == 'Pen Paper Assessment':
        ...
        pdf.cell(14, 0.5, 'GENDER', border='LR', align='C')
        ...
        pdf.cell(14, 4.5, '', border='LBR', align='C')  # Empty cell under GENDER
        pdf.cell(16, 4.5, 'Present/Absent', border='LBR', align='C')  # Empty cell under TAB ID
        ...
        if st.button("Click here to Generate PDFs"):
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                for record in result:
                    zone_name = record.get('zone Name', 'default_zone')
                    if zone_name not in zone_folders:
                        zone_folder = os.path.join(tmp_dir, zone_name)
                        os.makedirs(zone_folder, exist_ok=True)
                        zone_folders[zone_name] = zone_folder
                ...
                for index, record in enumerate(result):
                    ...
                    file_name = filename_template.format(school_name=school_name, zone_name=zone_name, block_name=block_name, grade=grade)
                    ...
                    pdf_path = os.path.join(zone_folders[zone_name], f'{file_name}.pdf')
                    pdf.output(pdf_path)
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    for zone_name, folder_path in zone_folders.items():
